...
# %cd /content/drive/your_project_folder
# %ls -lht
...
for i in range(0,X_test.shape[0]):
  ...
  for j in range (0,n_pred):
    pred=model7.predict(sequence[:,j:,:]) #predict next timestep with previous 1000 steps
    sequence=np.concatenate((sequence, pred.reshape(1, 1, 1)), axis=1) #add it to the sequence
...
predictions=model10.predict(X_valid)
predictions.shape

#obtain metrics of the test dataset
print('last_mse: ', last_time_step_mse(Y_valid,predictions.reshape((2000,10))))
print('mse: ', tf.reduce_mean(tf.keras.metrics.mean_squared_error(Y_valid,predictions.reshape((2000,10)))))
...
#obtain metrics of the validation dataset
print('last_timestep_mse: ', last_time_step_mse(Y_train[:,-1,:],predictions[:,-1,:]))
print('mse_last_series: ', tf.reduce_mean(tf.keras.metrics.mean_squared_error(Y_train[:,-1,:],predictions[:,-1,:])))
print('mse: ', tf.reduce_mean(tf.keras.metrics.mean_squared_error(Y_train,predictions)))
